    ...
    public function index_ssr(Request $request)
    {
        try {
            ...
            if ($request->q != 'null') {
                ...
                $mailTemplate = MailTemplate::where('type', 'email')->where('company_id', auth()->user()->company_id)->offset($offset)->limit($page_qty)->orderBy($request->sortField, $request->sortValue)->get();
                $mailTemplateAll = MailTemplate::where('type', 'email')->where('company_id', auth()->user()->company_id)->get();
            }
