...
class TriggerPropertyFeeBasedBillController extends Controller
{
    ...
    {
        $ownerPropertyFees = OwnerFees::where('owner_folio_id', $owner_folio_id)->where('property_id', $property_id)->with('feeSettings', 'feeSettings.account')->get();
        $rent = Receipt::where('to_folio_id', $owner_folio_id)->where('to_folio_type', 'Owner')->where('property_id', $property_id)->where('rent_amount', '>', 0)->first();
